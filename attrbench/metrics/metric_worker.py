...
class MetricWorker(Worker):
    def __init__(self, result_queue: mp.Queue, rank: int, world_size: int, all_processes_done: mp.Event,
        ...
        self.dataloader = DataLoader(self.dataset, sampler=sampler, batch_size=self.batch_size, num_workers=4)
        self.device = torch.device(self.rank)

    def _get_model(self) -> nn.Module:
        """
        Produces a model instance from the model_factory and sends it to the correct device
        :return: The model
        """
