...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, choices=["obj", "scene", "scene_only"])
    ...
    # Construct dataset
    ds = datasets.BAMDataset(args.batch_size, path.join(args.data_root, "bam"))
    train_dl = ds.get_dataloader(args.dataset, train=True)
    test_dl = ds.get_dataloader(args.dataset, train=False)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_epoch(model, train_dl, test_dl, optimizer, loss, args.device)
        if (epoch+1) % 5 == 0:
