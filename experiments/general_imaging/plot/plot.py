...
if __name__ == "__main__":
    ...
    parser.add_argument("in_file", type=str, nargs="?", default="../../../out/mnist.h5")
    ...
    for mode in ("single_dist", "median_dist", "std_dist"):
        ...
        for m_name in ("deletion", "insertion", "irof", "iiof", "sensitivity_n", "seg_sensitivity_n"):
            ...
            fig.savefig(path.join(args.out_dir, f"{m_name}-wsp-{mode}.png"), bbox_inches="tight")
            plt.close(fig)
        ...
        if mode != "std_dist":
            ...
            wsp = WilcoxonSummaryPlot(dfs)
            fig = wsp.render(figsize=(20, 15), glyph_scale=2000)
            fig.savefig(path.join(args.out_dir, f"deletion_until_flip-wsp-{mode}.png"), bbox_inches="tight")
            plt.close(fig)
        ...
        fig.savefig(path.join(args.out_dir, f"infidelity-wsp-{mode}.png"), bbox_inches="tight")
