...
# method_constructors = get_method_constructors(["InputXGradient", "Gradient", "IntegratedGradients", "Random"])
method_constructors = get_method_constructors(["IntegratedGradients"])
...
for key in method_constructors:
    ...
    for b in range(N_BATCHES):
        ...
        for i in range(1, N_INPUTS+1, INPUT_STEP):
            ...
            unraveled = np.unravel_index(to_mask.cpu(), samples.shape[1:]) # move to_mask back to cpu, even if device
            # is cuda, as this can cause OOM on GPU
            ...
            batch_result.append(model.predict(samples).gather(1, labels.reshape(-1, 1)).cpu().detach())
            if i in example_idxs and b == 0:
                method_examples.append(np.array(samples[0].cpu().detach().numpy()))
